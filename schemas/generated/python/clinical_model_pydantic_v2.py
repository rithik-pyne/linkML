...
linkml_meta = LinkMLMeta({'comments': ['v2.0.0: Refactored ResponseAssessment into ImagingResponse, '
                  'MolecularResponse, ClinicalResponse',
                  'Breaking change: ResponseAssessment deprecated - use '
                  'specialized response classes',
                  'See RESPONSE_TABLE_REFACTOR_SPEC.md for migration guide'],
     'default_prefix': 'clinical_model',
...
class ClinicalEventTypeEnum(str, Enum):
    """
    Type of clinical outcome event documented in ClinicalResponse
    """
    Progression = "Progression"
    """
    ...
    """
    Histologic transformation (e.g. SCLC)
    """
...
class MolecularTest(ConfiguredBaseModel):
    ...
    molecular_test_id: str = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['MolecularTest', 'Mutation', 'MolecularResponse']} })
    biopsy_id: str = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['Biopsy', 'MolecularTest']} })
...
class Treatment(ConfiguredBaseModel):
    ...
    treatment_id: str = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['Treatment',
                       'ImagingResponse',
                       'MolecularResponse',
                       'ClinicalResponse']} })
    patient_id: str = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['Patient',
...
class ImagingResponse(ConfiguredBaseModel):
    """
    Treatment response assessment based on imaging studies (RECIST criteria). Grain - one row per imaging study with documented radiologist assessment.
    """
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta({'comments': ['Links to source ImagingStudy via required FK',
                      'Used for RECIST-based response tracking over time',
                      'treatment_id nullable for pre-treatment baseline scans'],
         'from_schema': 'https://ngdx.org/clinical_model',
         'slot_usage': {'assessment_date': {'name': 'assessment_date',
                                            'required': True},
                        'assessment_type': {'name': 'assessment_type',
                                            'required': True},
                        'imaging_response_id': {'identifier': True,
                                                'name': 'imaging_response_id',
                                                'range': 'string',
                                                'required': True},
                        'imaging_study_id': {'description': 'Source imaging study '
                                                            '(REQUIRED - establishes '
                                                            'lineage)',
                                             'identifier': False,
                                             'name': 'imaging_study_id',
                                             'required': True},
                        'new_lesions_present': {'name': 'new_lesions_present',
                                                'required': False},
                        'patient_id': {'description': 'Conformed dimension for '
                                                      'drill-across queries',
                                       'identifier': False,
                                       'name': 'patient_id',
                                       'required': True},
                        'percent_change_from_baseline': {'name': 'percent_change_from_baseline',
                                                         'required': False},
                        'recist_response': {'name': 'recist_response',
                                            'required': False},
                        'sum_target_lesions_mm': {'name': 'sum_target_lesions_mm',
                                                  'required': False},
                        'treatment_id': {'description': 'Treatment context (NULL for '
                                                        'pre-treatment baseline)',
                                         'identifier': False,
                                         'name': 'treatment_id',
                                         'required': False}}})

    imaging_response_id: str = Field(default=..., description="""Primary key for ImagingResponse. Format - IR-{patient_num}-{counter} e.g. IR-001-001""", json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse']} })
    imaging_study_id: str = Field(default=..., description="""Source imaging study (REQUIRED - establishes lineage)""", json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse', 'ImagingStudy']} })
    patient_id: str = Field(default=..., description="""Conformed dimension for drill-across queries""", json_schema_extra = { "linkml_meta": {'domain_of': ['Patient',
                       'Biopsy',
                       'Treatment',
                       'ImagingResponse',
                       'MolecularResponse',
                       ...
                       'ClinicalAssessment',
                       'ImagingStudy']} })
    treatment_id: Optional[str] = Field(default=None, description="""Treatment context (NULL for pre-treatment baseline)""", json_schema_extra = { "linkml_meta": {'domain_of': ['Treatment',
                       'ImagingResponse',
                       'MolecularResponse',
                       'ClinicalResponse']} })
    assessment_date: date = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse', 'MolecularResponse', 'ClinicalAssessment']} })
    assessment_type: AssessmentTypeEnum = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse', 'MolecularResponse']} })
    recist_response: Optional[RECISTResponseEnum] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse']} })
    sum_target_lesions_mm: Optional[float] = Field(default=None, ge=0, le=500, json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse']} })
    percent_change_from_baseline: Optional[float] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse']} })
    new_lesions_present: Optional[bool] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse']} })

    @field_validator('imaging_response_id')
    def pattern_imaging_response_id(cls, v):
        pattern=re.compile(r"^IR-[0-9]{3}-[0-9]{3}$")
        if isinstance(v, list):
            for element in v:
                if isinstance(element, str) and not pattern.match(element):
                    err_msg = f"Invalid imaging_response_id format: {element}"
                    raise ValueError(err_msg)
        elif isinstance(v, str) and not pattern.match(v):
            err_msg = f"Invalid imaging_response_id format: {v}"
            raise ValueError(err_msg)
        return v
...
class MolecularResponse(ConfiguredBaseModel):
    """
    Treatment response assessment based on molecular testing (ctDNA/VAF tracking). Grain - one row per molecular test with documented ctDNA response.
    """
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta({'comments': ['Links to source MolecularTest via required FK',
                      'Used for serial ctDNA monitoring and VAF trend analysis',
                      'treatment_id nullable for pre-treatment baseline tests'],
         'from_schema': 'https://ngdx.org/clinical_model',
         'slot_usage': {'assessment_date': {'name': 'assessment_date',
                                            'required': True},
                        'assessment_type': {'name': 'assessment_type',
                                            'required': True},
                        'ctdna_mutation_cleared': {'name': 'ctdna_mutation_cleared',
                                                   'required': False},
                        'ctdna_tumor_fraction_percent': {'name': 'ctdna_tumor_fraction_percent',
                                                         'required': False},
                        'ctdna_vaf_percent': {'name': 'ctdna_vaf_percent',
                                              'required': False},
                        'molecular_response_id': {'identifier': True,
                                                  'name': 'molecular_response_id',
                                                  'range': 'string',
                                                  'required': True},
                        'molecular_test_id': {'description': 'Source molecular test '
                                                             '(REQUIRED - establishes '
                                                             'lineage)',
                                              'identifier': False,
                                              'name': 'molecular_test_id',
    ...
    molecular_response_id: str = Field(default=..., description="""Primary key for MolecularResponse. Format - MR-{patient_num}-{counter} e.g. MR-001-001""", json_schema_extra = { "linkml_meta": {'domain_of': ['MolecularResponse']} })
    molecular_test_id: str = Field(default=..., description="""Source molecular test (REQUIRED - establishes lineage)""", json_schema_extra = { "linkml_meta": {'domain_of': ['MolecularTest', 'Mutation', 'MolecularResponse']} })
    patient_id: str = Field(default=..., description="""Conformed dimension for drill-across queries""", json_schema_extra = { "linkml_meta": {'domain_of': ['Patient',
                       'Biopsy',
                       'Treatment',
                       'ImagingResponse',
    ...
    assessment_date: date = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse', 'MolecularResponse', 'ClinicalAssessment']} })
    assessment_type: AssessmentTypeEnum = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse', 'MolecularResponse']} })
    ctdna_vaf_percent: Optional[float] = Field(default=None, ge=0.0, le=100.0, json_schema_extra = { "linkml_meta": {'domain_of': ['MolecularResponse']} })
    ctdna_tumor_fraction_percent: Optional[float] = Field(default=None, ge=0.0, le=100.0, json_schema_extra = { "linkml_meta": {'domain_of': ['MolecularResponse']} })
    ctdna_mutation_cleared: Optional[bool] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['MolecularResponse']} })

    @field_validator('molecular_response_id')
    def pattern_molecular_response_id(cls, v):
        pattern=re.compile(r"^MR-[0-9]{3}-[0-9]{3}$")
        if isinstance(v, list):
            for element in v:
                if isinstance(element, str) and not pattern.match(element):
                    err_msg = f"Invalid molecular_response_id format: {element}"
                    raise ValueError(err_msg)
        elif isinstance(v, str) and not pattern.match(v):
            err_msg = f"Invalid molecular_response_id format: {v}"
            raise ValueError(err_msg)
        return v
...
class ClinicalResponse(ConfiguredBaseModel):
    """
    Clinical outcome events (progression, resistance, histologic transformation). Grain - one row per documented clinical outcome event. Does NOT link to specific imaging/molecular tests - represents MDT consensus.
    """
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta({'comments': ['Independent clinical event documentation',
                      'May be informed by imaging/molecular data but grain is the MDT '
                      'decision',
                      'event_date is when progression was declared, may differ from '
                      'test date',
                      'treatment_id may be null for post-treatment events'],
         'from_schema': 'https://ngdx.org/clinical_model',
         'slot_usage': {'clinical_response_id': {'identifier': True,
                                                 'name': 'clinical_response_id',
                                                 'range': 'string',
                                                 'required': True},
                        'event_date': {'description': 'Date event was documented (may '
                                                      'differ from test date)',
                                       'name': 'event_date',
                                       'required': True},
                        'event_type': {'name': 'event_type', 'required': True},
                        'histologic_transformation': {'name': 'histologic_transformation',
                                                      'required': False},
                        'patient_id': {'description': 'Conformed dimension for '
                                                      'drill-across queries',
                                       'identifier': False,
                                       'name': 'patient_id',
                                       'required': True},
                        'progression_detected': {'name': 'progression_detected',
                                                 'required': True},
                        'progression_type': {'name': 'progression_type',
                                             'required': False},
                        'resistance_mechanism': {'name': 'resistance_mechanism',
                                                 'required': False},
                        'resistance_mutation_detected': {'name': 'resistance_mutation_detected',
                                                         'required': False},
                        'time_to_progression_months': {'name': 'time_to_progression_months',
                                                       'required': False},
                        'treatment_id': {'description': 'Treatment context (may be '
                                                        'post-treatment)',
                                         'identifier': False,
                                         'name': 'treatment_id',
                                         'required': False}}})

    clinical_response_id: str = Field(default=..., description="""Primary key for ClinicalResponse. Format - CR-{patient_num}-{counter} e.g. CR-001-001""", json_schema_extra = { "linkml_meta": {'domain_of': ['ClinicalResponse']} })
    patient_id: str = Field(default=..., description="""Conformed dimension for drill-across queries""", json_schema_extra = { "linkml_meta": {'domain_of': ['Patient',
                       'Biopsy',
                       'Treatment',
                       'ImagingResponse',
                       ...
                       'ClinicalAssessment',
                       'ImagingStudy']} })
    treatment_id: Optional[str] = Field(default=None, description="""Treatment context (may be post-treatment)""", json_schema_extra = { "linkml_meta": {'domain_of': ['Treatment',
                       'ImagingResponse',
                       'MolecularResponse',
                       'ClinicalResponse']} })
    event_date: date = Field(default=..., description="""Date event was documented (may differ from test date)""", json_schema_extra = { "linkml_meta": {'domain_of': ['ClinicalResponse']} })
    event_type: ClinicalEventTypeEnum = Field(default=..., description="""Type of clinical event (Progression, Resistance, Transformation)""", json_schema_extra = { "linkml_meta": {'domain_of': ['ClinicalResponse']} })
    progression_detected: bool = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['ClinicalResponse']} })
    progression_type: Optional[ProgressionTypeEnum] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['ClinicalResponse']} })
    time_to_progression_months: Optional[float] = Field(default=None, ge=0, json_schema_extra = { "linkml_meta": {'domain_of': ['ClinicalResponse']} })
    resistance_mutation_detected: Optional[bool] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['ClinicalResponse']} })
    resistance_mechanism: Optional[str] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['ClinicalResponse']} })
    histologic_transformation: Optional[bool] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['ClinicalResponse']} })

    @field_validator('clinical_response_id')
    def pattern_clinical_response_id(cls, v):
        pattern=re.compile(r"^CR-[0-9]{3}-[0-9]{3}$")
        if isinstance(v, list):
            for element in v:
                if isinstance(element, str) and not pattern.match(element):
                    err_msg = f"Invalid clinical_response_id format: {element}"
                    raise ValueError(err_msg)
        elif isinstance(v, str) and not pattern.match(v):
            err_msg = f"Invalid clinical_response_id format: {v}"
            raise ValueError(err_msg)
        return v
...
class ClinicalAssessment(ConfiguredBaseModel):
    ...
    ecog_status: Optional[int] = Field(default=None, ge=0, le=5, json_schema_extra = { "linkml_meta": {'domain_of': ['ClinicalAssessment']} })
...
class ImagingStudy(ConfiguredBaseModel):
    ...
    imaging_study_id: str = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['ImagingResponse', 'ImagingStudy']} })
    patient_id: str = Field(default=..., json_schema_extra = { "linkml_meta": {'domain_of': ['Patient',
                       'Biopsy',
                       'Treatment',
                       'ImagingResponse',
...
ImagingResponse.model_rebuild()
MolecularResponse.model_rebuild()
ClinicalResponse.model_rebuild()
